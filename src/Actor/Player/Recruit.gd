...
func _physics_process(delta):
	if disabled != true:
		if colliding == true: #skip this entire thing if we're in debug mode
			...
			var effect_pos = $BulletVector.position
...
func get_move_dir() -> Vector2:
	return Vector2(
		Input.get_action_strength("move_right") - Input.get_action_strength("move_left"),
		-1.0 if Input.is_action_just_pressed("jump") and $ForgivenessTimer.time_left > 0 or Input.is_action_just_pressed("jump") and is_on_floor() else 0.0
		)

func get_look_dir(look_rot) -> Vector2:
	if look_rot == 90: #Left
		return Vector2(-1, 0)
	elif look_rot == 270: #Right
		return Vector2(1, 0)
...
func calculate_move_velocity(linear_velocity: Vector2, move_dir, look_dir, special_dir, speed, is_jump_interrupted, is_dodge_interrupted) -> Vector2:
	...
	if special_dir == Vector2.ZERO: #normal, no special
		...
		if is_jump_interrupted:
			out.y = 0.0
	elif knockback == true: #with special, knockback
		if move_dir.x != 0:
			out.x = speed.x * ((move_dir.x * knockback_mod.x * special_dir.x)/2) 
		else:
			out.x = speed.x * (special_dir.x * knockback_mod.x)
		
		out.y += gravity * get_physics_process_delta_time()
		
		if move_dir.y < 0 or special_dir.y < 0: 
			if move_dir.y != 0:
				out.y = speed.y * ((move_dir.y * knockback_mod.y * special_dir.y)/2)
			else:
				out.y = speed.y * (special_dir.y * knockback_mod.y)
		if is_jump_interrupted:
			out.y = 0.0
...
func animate(move_dir, look_dir):
