...
func _ready():
	...
	for s in spikes_left_tiles:
		...
		if get_cellv(above) == -1: #if this isn't the first cell, return
			var spikes_size = 1
			while get_cellv(below) == 0:
				spikes_size +=1
				below.y += 1
			
			var spikes_left = SPIKESLEFT.instance()
	...
	for s in spikes_right_tiles:
		...
		if get_cellv(above) == -1: #if this isn't the first cell, return
			var spikes_size = 1
			while get_cellv(below) == 1:
				spikes_size +=1
				below.y += 1
		
			var spikes_right = SPIKESRIGHT.instance()
	...
	for s in spikes_up_tiles:
		...
		if get_cellv(left) == -1: #if this isn't the first cell, return
			var spikes_size = 1
			while get_cellv(right) == 2:
				spikes_size +=1
				right.x += 1
			
			var spikes_up = SPIKESUP.instance()
	...
	for s in spikes_down_tiles:
		...
		if get_cellv(left) == -1: #if this isn't the first cell, return
			var spikes_size = 1
			while get_cellv(right) == 3:
				spikes_size +=1
				right.x += 1
			
			var spikes_down = SPIKESDOWN.instance()
